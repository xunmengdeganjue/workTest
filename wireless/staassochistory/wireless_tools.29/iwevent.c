...
static void event_to_custom( const u8 *custom,int len,int flags){
...
//#define MGMT_FRAM_TAG_SIZE 30 /* hardcoded in driver */
	...
	if(len<sizeof(struct ieee80211_hdr)){
		printf("\033[36m[%s]:[%d]:the len of the custom is[%d] too short!should bigger than [%d]\033[0m\n",__func__,__LINE__,len,sizeof(struct ieee80211_hdr));
		return ;
	}
	...
	printf("\033[35mNICK:[%s]:subtype 0x%x len [\033[32m%d\033[0m]\033[0m\n",__func__,stype,len);
...
#define MGMT_FRAM_TAG_SIZE 30 /* hardcoded in driver */
	...
	case IWEVCUSTOM:/*NICK:################################CUSTOM EVENT*/
	{
		...
		printf("\033[32mNICK:[%s]:##########CUSTOM :event->cmd=0x%x,len=%d\033[0m\n",__func__,event->cmd,event->len);
		
		printf("\033[32mNICK:[%s]:CUSTOM EVENT IEs:%s\033[0m\n",__func__,
		...
		if(custom){
			...
			if (strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) 
			{
				...
				event_to_custom((u8*)custom+MGMT_FRAM_TAG_SIZE,len,event->u.data.flags);
	...
	printf("\033[33mNICK:Expired AP_ADDR %s\033[0m\n",iw_saether_ntop(&event->u.ap_addr, buffer));
	memset(buffer,0x00,sizeof(buffer));
	printf("\033[33mNICK:Expired AP_ADDR %s\033[0m\n",iw_saether_ntop(&event->u.essid, buffer)); 
      break;
    case SIOCGIWTHRSPY:
      {
