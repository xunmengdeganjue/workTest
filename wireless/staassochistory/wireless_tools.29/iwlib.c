			...
			int			we_version)
{
...
#ifdef DEBUG
  printf("DBG - stream->current = %p, stream->value = %s, stream->end = %p\n",
	 stream->current, stream->value, stream->end);
	if(stream->value){
  		printf("\033[35mNICK:the stream->value is [%s]\033[0m\n",stream->value);
	}else{
		printf("\033[35mNICK:the stream->value is [************%s***********]\033[0m\n",stream->value);
	}
  ...
  memcpy((char *) iwe, stream->current, IW_EV_LCP_PK_LEN);
	printf("\033[31mNICK:######NOW THE ADDR OF THE iwe is [%p]\033[0m\n",iwe);

#ifdef DEBUG
  printf("DBG - iwe->cmd =\033[31m 0x%X\033[0m, iwe->len = \033[31m%d\033[0m\n",
	 iwe->cmd, iwe->len);
#endif
  ...
  printf("\033[33mNICK---the we_version = %d\033[0m\n",we_version);
  if((we_version <= 18) && (event_type == IW_HEADER_TYPE_POINT)){
    event_len += IW_EV_POINT_OFF;
	printf("\033[33mNICK---the IW_EV_POINT_OFF=%d\033[0m\n",IW_EV_POINT_OFF);
	printf("\033[33mNICK---the event_len=event_len+IW_EV_POINT_OFF=%d\033[0m\n",event_len);
  }
  ...
  if(event_len <= IW_EV_LCP_PK_LEN)
    {
      /* Skip to next event */
	printf("\033[33mNICK---[the event_len=%d <= 4] So Skip to next event\033[0m\n",event_len); 
...
#ifdef DEBUG
  printf("DBG - event_type = \033[32m%d\033[0m, event_len = \033[32m%d\033[0m, pointer = \033[32m%p\033[0m\n",
  ...
  if((we_version > 18) && (event_type == IW_HEADER_TYPE_POINT)){
	...
	printf("\033[31mNICK---the IW_EV_POINT_OFF=%d\033[0m\n",IW_EV_POINT_OFF);
	printf("\033[31mNICK---the IW_EV_LCP_LEN=%d\033[0m\n",IW_EV_LCP_LEN);
	printf("\033[31mNICK---the event_len=%d\033[0m\n",event_len);
  ...
  if(event_type == IW_HEADER_TYPE_POINT)
      ...
      if(extra_len > 0)
	{
	  ...
	  if(descr == NULL)
		...
		{
			printf("\033[31mNICK-2862--the token_len=%d\033[0m\n",token_len);
		...
		printf("\033[31mNICK-2882--the token_len=%d\033[0m\n",token_len);
		printf("\033[31mNICK-2883--the extra_len=%d\033[0m\n",extra_len);
		
	      /* Discard bogus events which advertise more tokens than
	       * what they carry... */
	      if(token_len > extra_len){
		  	
		  	printf("\033[33mNICK-2883--2887-discard paylod\033[0m\n");
			iwe->u.data.pointer = NULL;	/* Discard paylod */
		 ...
		 && !(descr->flags & IW_DESCR_FLAG_NOMAX)){
		 	...
		 	printf("\033[33mNICK-2883--2895-discard paylod\033[0m\n");
			iwe->u.data.pointer = NULL;	/* Discard paylod */
	      ...
	      if(iwe->u.data.length < descr->min_tokens){
		  	...
		  	printf("\033[33mNICK-2901--2887-discard paylod\033[0m\n");
			iwe->u.data.pointer = NULL;	/* Discard paylod */

	      	}
		printf("\033[32mNICK---Length of the iwe->u.data.length is[%d]\033[0m\n",iwe->u.data.length);
		
#ifdef DEBUG
	      printf("DBG - \033[36mextra_len = %d, token_len = %d, token = %d, max = %d, min = %d\033[0m\n",
