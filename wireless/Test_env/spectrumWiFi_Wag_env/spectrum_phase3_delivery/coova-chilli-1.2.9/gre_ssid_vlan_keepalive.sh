...
brctl delif br-tap eth0.5

if [ -z ${IP6+x} ];
then
	ip addr del $BR_IP/24 dev eth0.5
	ip addr del $SEC_LOCAL_EP/24 dev eth0.5
else
	ip -6 addr del $BR_IP/64 dev eth0.5
	ip -6 addr del $SEC_LOCAL_EP/64 dev eth0.5
fi
...
then
	...
	ip -6 link add $GRE_NAME type ip6gretap local $LOCAL_EP remote $REMOTE_EP
	ip -6 link add $SEC_GRE_NAME type ip6gretap local $SEC_LOCAL_EP remote $REMOTE_EP

	echo "Bringing $GRE_NAME up"
	ip -6 link set dev $GRE_NAME up 
...
echo "Creating eth0.5"
vconfig add eth0 5
sleep 1
echo "Creating br-com"
brctl addbr br-com
echo "Assigning $BR_IP to eth0.5"

if [ -z ${IP6+x} ];
then
	ip addr add $BR_IP/24 dev eth0.5
	ip addr add $SEC_LOCAL_EP/24 dev eth0.5
else
	ip -6 addr add $BR_IP/64 dev eth0.5
	ip -6 addr add $SEC_LOCAL_EP/64 dev eth0.5
fi
...
#radiusd -Xx &
#freeradius -Xx & # Added By Dheeraj Because i have freeradius version 2.1.12
