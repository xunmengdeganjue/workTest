...
proto_dhcpv6_setup() {
	...
	local requestedoptions="$(uci_get ipv6.@native[0].requestedoptions)" 
	[ "$requestedoptions" != "" ] && append opts "-r$requestedoptions" 
	...
	proto_run_command "$config" odhcp6c -e -v -N try \
