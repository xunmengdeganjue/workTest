...
#endif 
	return sprintf(p_self->p_req_buffer,"http://checkip.dyndns.com/");
}
...
static RC_TYPE do_ip_server_transaction(DYN_DNS_CLIENT *p_self)
{
	...
	if (rc != RC_OK)
	{
		printf("\033[31mNICKLI:http_client_init error!\033[0m\n");
		return rc;
	}

	do
	{
		printf("\033[31mprepare request for IP server\033[0m\n");
...
RC_TYPE dyn_dns_update_ip(DYN_DNS_CLIENT *p_self)
{
	...
	do
	{
		...
		printf("extract my IP, check if different than previous one\n");
